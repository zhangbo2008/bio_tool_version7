...
def save():
        ...
        if 1:
            ...
            with open('output.bio','w') as f:
                f.writelines(jieguo)
            with open('output.bio_byte','w') as f:
                f.writelines(jieguo)
            #对原文和结果进行处理即可.
            all_for_word=[]
            for i in range(len(yuanwen)):
                ...
                aaaaa = yuanwen[i]
                jieguo_word=[]
                for j in range(len(aaaaa)):
                    dangqianzifu = aaaaa[j]
                    if j == 0 and dangqianzifu != ' ':
                        #那么说明他是一个需要标记的
                        jieguo_word.append(jieguo_help[i][j])
                    if j == 0 and dangqianzifu == ' ':
                        pass
                    if j > 0 and dangqianzifu != ' ' and aaaaa[j - 1] == ' ':
                        #如果这个字符是可以开始字符.那么我们家入.
                        jieguo_word.append(jieguo_help[i][j])
                jieguo_word=' '.join(jieguo_word)        
                all_for_word.append(jieguo_word)
            jieguo_for_word=[i+'\n' for i in all_for_word]
            with open('output.bio_word','w') as f:
                f.writelines(jieguo_for_word)

#==============下面我们添加word版本.#=也就是英文常用的ner标注方法.
